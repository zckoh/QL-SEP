# -*- coding: utf-8 -*-
"""
Created on Thu Feb 08 15:41:40 2018
...
node1_actual_shared= 0
node2_actual_shared = 0
node1_actual_shared_lst = []
node2_actual_shared_lst = []
node1_actual_shared_avg_lst = []
node2_actual_shared_avg_lst = []


node1_actual_requested = 0
node2_actual_requested = 0
node1_actual_requested_lst = []
node2_actual_requested_lst = []
node1_actual_requested_avg_lst = []
node2_actual_requested_avg_lst = []



MAPE_QLSEP_lst_b1 = []
MAPE_QLSEP_lst_b2 = []
shared_param = np.linspace(0,48,num=24)
cmpare = []
...
for k in range(0,24):
    ...
    for x in range(0,days):
        ...
        node1_actual_requested_lst.append(node1_actual_requested)
        node2_actual_requested_lst.append(node2_actual_requested)
        
        node1_actual_shared_lst.append(node1_actual_shared)
        node2_actual_shared_lst.append(node2_actual_shared)
        ...
        total_shared_counts = 0
        if(k<=23 and x == 19):
            cmpare.append(node1.QLSEP_val[x])
        
    node1_actual_shared_avg_lst.append(np.average(node1_actual_shared_lst))
    node2_actual_shared_avg_lst.append(np.average(node2_actual_shared_lst))
    
    node1_actual_requested_avg_lst.append(np.average(node1_actual_requested_lst))
    node2_actual_requested_avg_lst.append(np.average(node2_actual_requested_lst))
    #print np.average(node1_actual_shared_lst)
    ...
    MAPE_QLSEP_lst_b1.append(MAPE_QLSEP_overall)
    
    """Box 2"""
    a = np.amax(lux_tmp_b2)
    b = a*0.05

    MAPE_QLSEP_overall = 0
    QLSEP_MAPE_N_COUNTER = 0
    #print predicted_val_WCMA[index]
    for i in range(len(lux_tmp_b2)):
        if(lux_tmp_b2[i]!=0):
            if(lux_tmp_b2[i]>b):
                QLSEP_MAPE_N_COUNTER += 1
                MAPE_QLSEP_overall += abs((lux_tmp_b2[i]-lux_pre_QLSEP_tmp_b2[i])/lux_tmp_b2[i])
                    
    MAPE_QLSEP_overall = MAPE_QLSEP_overall*100/QLSEP_MAPE_N_COUNTER
    MAPE_QLSEP_lst_b2.append(MAPE_QLSEP_overall)

apple = np.array(cmpare[0])
beta = np.array(cmpare[1])
...
ax.plot(shared_param,MAPE_QLSEP_lst_b1,'x',label = 'Maximum allowed Q-values')
ax.plot(node1_actual_shared_avg_lst,MAPE_QLSEP_lst_b1,'x',label = 'Actual shared Q-values (average)')
ax.plot(node1_actual_requested_avg_lst,MAPE_QLSEP_lst_b1,'x',label = 'Actual requested Q-values (average)')
...
plt.grid()
plt.xlabel('No. of Q-values shared in 1 day')
plt.ylabel('MAPE(%)')
plt.title('Prediction Accuracy vs No. of Q-values shared in 1 day (Box 1)')

plt.figure(2)
fig, ax = plt.subplots(figsize=(7,4))
ax.plot(shared_param,MAPE_QLSEP_lst_b2,'x',label = 'Maximum allowed Q-values')
ax.plot(node2_actual_shared_avg_lst,MAPE_QLSEP_lst_b2,'x',label = 'Actual shared Q-values')
ax.plot(node2_actual_requested_avg_lst,MAPE_QLSEP_lst_b2,'x',label = 'Actual requested Q-values')
"""
legend = ax.legend(loc='upper right', shadow=True)
frame = legend.get_frame()
frame.set_facecolor('1.0')
...
plt.xlabel('No. of Q-values shared in 1 day')
plt.ylabel('MAPE(%)')
plt.title('Prediction Accuracy vs No. of Q-values shared in 1 day (Box 2)')
...
ax.plot(time,cmpare[0],'rx',label= 's')
